...
- **FR-E-006 — 设备屏幕与三主页**：AMOLED 提供木鱼页、经文页、统计页与下滑设置页；顶部状态栏常驻电量/充电、4G、同步/待同步。木鱼页固定 7 字带：只呈现最近已诵内容、空位低对比度占位、**不预览未来经文**、新字右入旧字左移、标点随相邻字同一步出现但不消耗敲击、显示本轮进度与今日计数；经文页大号当前字 + 少量前后文 + 本轮进度；统计页仅今日与累计。未校时显示「待校时」；熄屏实体敲击短暂亮屏。（→AD-2/6/19）
...
- **FR-F-002 — 经文呈现页**：小程序不显示电子木鱼、不产生正式敲击；阅读行自动滚动 + 当前字聚焦呈现 backend 已确认内容，顺序不越权、不预览未来。（→AD-2/17/19）
- **FR-F-003 — 在线逐字动画**：每个 backend 已确认汉字按顺序展示一次；动画落后时动态加速，不跳过、合并或重复字符；渲染环节不单独突破 SM-3 端到端 1 秒口径。（→AD-17/19）
...
- 木鱼页：电子木鱼为唯一可计数触摸区；7 字形位置，**只展示已诵内容、未填充位低对比度占位、不预览未来经文**；标点随附同一步插入且正式计数只 +1；动画积压时缩短位移时间、清空后恢复节奏；同时显示本轮进度与今日计数，同步未确认用同步中/待同步标记。
- 经文页：大号当前字 + 少量前后文 + 本轮进度；MVP 无选经入口。统计页：仅今日 + 累计。
...
本项目尚无 bmad-ux 契约（无 `DESIGN.md`/`EXPERIENCE.md`）。设备端视觉/交互细节以 09-08 简报附录承接（已并入上方「设备端 UX 参考」附加需求），不另设 UX-DR 清单。
...
- **实现要点**：单 UI 任务内多 story 顺序落地；经文页整页内容（大号当前字+前后文+本轮进度）、木鱼页「本轮进度+今日计数+反剧透占位」等 UX 细节以 09-08 附录 §5 为实现参考。设置默认值（音量 50/中亮度/15s 熄屏）写死入 UI 首帧；固件构建期嵌入 S0 canonical 经文并校验 `scripture_version` 一致（P4，配合 FR-B-004/AD-4）。
...
- Given 页面切换，When 任意主页可见，Then 顶部状态栏常驻显示电量/充电、4G、同步/待同步。
...
- **As a** 诵经者，**I want** 木鱼页以固定 7 字带呈现最近已诵内容，**So that** 诵经连续、不预览未来经文。
...
- Given 本地有未确认差值或尚未同步，When 木鱼页展示本轮进度/今日计数，Then 以「同步中/待同步」等非正式状态显式标记（AD-2），本地镜像不冒充已确认。
...
#### Story E3.4 · 经文页内容契约（大号当前字 + 前后文 + 本轮进度）

- **layer**：`Embedded` ｜ **绑定**：FR-E-006（经文页）、AD-19、09-08 附录 §5.3
- **As a** 诵经者，**I want** 经文页以大号当前字配少量前后文与进度呈现，**So that** 诵经时能看清当前进度而不被打断。

**Acceptance Criteria:**
- Given 经文页可见，When 引擎游标变化，Then 大号显示当前可消费字，前/后展示少量已诵/已临近内容（不预览未诵未来字）。
- Given 页面加载，When 渲染，Then 显示本轮进度；MVP 不出现选经/导入入口。
...
- **As a** 诵经者，**I want** 设备统计页只显示今日与累计，未校时时显「待校时」，**So that** 端上不越权承担近 7/30 日与连续天数统计。

**Acceptance Criteria:**
- Given 统计页可见，When 渲染，Then 只显示今日敲击数与累计敲击数（近 7/30 日与连续天数不在设备端，归小程序记录页）。
...
- Given 电量计 CW2015 就绪，When 轮询电量，Then 状态栏展示电量百分比并在低电量时明确低打扰提示。
...
**Acceptance Criteria:**
- Given 引擎（E2.7）判定本轮完成，When 显示完成遮罩，Then 遮罩期间所有实体/触摸输入被忽略（复用 E2.3 输入抑制），不增加本地或云端计数。
...
- **实现要点**：首 story = frontend 工程骨架（uni-app Vue3+TS+Vite+Pinia + 单一 `VITE_API_BASE_URL` + 统一 `api/request` + 登录路由），**S4 复用本骨架**；**mock 依据 = S0 的 frontend-facing API 契约**（P1），不依赖 S1 时序；动画落后动态加速不丢字符；增加「构建期嵌入 canonical 经文并校验 `scripture_version`」story（引用 S0，P4）。
...
#### Story S3.2 · 经文呈现页（阅读行 + 当前字聚焦）

- **layer**：`Software·Frontend` ｜ **绑定**：FR-F-002、AD-2/19
- **As a** 诵经者，**I want** 主界面以阅读行自动滚动 + 当前字聚焦呈现已确认经文，**So that** 呈现纯粹、可读，且不产生正式敲击。

**Acceptance Criteria:**
- Given 主界面，When 渲染，Then 无电子木鱼/点击敲击入口，不产生任何正式输入事件。
- Given 已确认差量，When 展示，Then 阅读行自动滚动并聚焦当前字，只呈现 backend 已确认字符，顺序不越权、不预览未来（AD-19）。
- Given 视觉设计，When 对齐规范，Then 允许与设备端不同，但数据/顺序以 backend 为准。

#### Story S3.3 · 在线逐字动画（不跳/不合并/不重复）

- **layer**：`Software·Frontend` ｜ **绑定**：FR-F-003、AD-17/19、SM-3（渲染侧）
- **As a** 诵经者，**I want** 每个已确认汉字按顺序展示一次、落后时动态加速，**So that** 快速连敲也能流畅跟诵而不丢字。

**Acceptance Criteria:**
- Given 实时差量到达（序号 > 快照水位 S），When 逐字展示，Then 每个汉字按 backend 确认顺序显示一次，不跳过、不合并、不重复（FR-F-003）。
...
**Acceptance Criteria:**
- Given 存在离线积压差量，When 打开小程序，Then 按 backend 差量顺序回放（不跳不重），回放期间新到达事件进入队列。
...
**Acceptance Criteria:**
- Given backend 完成确认到达，When 前端收到，Then 显示礼花与「从头开始/退出」弹窗（FR-F-005）。
- Given 尚未收到确认，When 本轮本地看似结束，Then 不提前声称完成、不显示礼花。
...
- **As a** 诵经者，**I want** 记录页展示今日/近 7/30 日/累计/连续天数，**So that** 我回看自己的诵经历史。

**Acceptance Criteria:**
- Given 记录页，When 查询统计，Then 展示今日、近 7 日、近 30 日、累计与连续天数，全部来自 backend 已确认数据（按 S0.2 API 契约）。
