...
- QMI8658A 上板但默认不启用，只接 INT1→IO41；INT2 不接，GPIO45 不用于 IMU。
- Air780EGP 按木鱼专属 GPIO 合同重映射，保留屏幕和音频的 `legbot_watch` 资源：UART IO43/44、DTR IO10、RST IO15、NET_STATUS 兼容位 IO16、PVDF 比较器 IO11、RGB IO3；`IO8` 现为 PWR 按键输入（读 LTC2954 的 `PWR_INT`，开漏低有效、按下期间持续为低，非脉冲；原 `BQ_OTG_EN` 方案作废——BQ25895 的 OTG 引脚已硬件接地，不占 GPIO），M100 `GNSS_VCC` 留 NC/测试点。
...
- 上一版两轨方向稿和换色候选均未签收；Stage 3 改为设备 10 个、小程序 10 个真正不同的视觉候选，作者两端分别选 1 个后才进入 15/14 帧闭包。
- 候选共享行为和数据；构图、字体层级、材质、进度表现、光影/签名细节中至少三个非颜色维度须不同。
- 设备状态栏采用可复用组件：四信号左置、电池符号+百分比右置；屏幕圆弧安全区与实际渲染必须对账。
- “本轮”不作为主页面文案；使用“心经进度”“今日敲击 N 次”“累计敲击 M 次”。完成与新 `round_id` 语义仍由 PRD/架构治理。
- 小程序正文为连续确认前缀，禁止手工按演示行拆开、重复句号或独立下划线。唯一进度组件承载字数、百分比和进度条。
- 最终 UI 文案硬禁令以根 `AGENTS.md` §3.7 为准；AI/作者提示、节点标识和调试说明不得进入交付页面。
