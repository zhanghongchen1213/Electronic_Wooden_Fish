...
- Air780EGP 按木鱼专属 GPIO 合同重映射，保留屏幕和音频的 `legbot_watch` 资源：UART IO43/44、DTR IO10、RST IO15、NET_STATUS 兼容位 IO16、PVDF 比较器 IO11、RGB IO3；`IO8` 现为 PWR 按键输入（读 TPS3424 的 `PWR_INT` 脉冲；原 `BQ_OTG_EN` 方案作废——BQ25895 的 OTG 引脚已硬件接地，不占 GPIO），M100 `GNSS_VCC` 留 NC/测试点。
