...
> **更正（2026-09-17）**：PWR 输入已从 IO46 移到 **IO8**——读 TPS3424 的 `PWR_INT` 脉冲（短按 50 ms／长按 100 ms），固件必须用边沿中断、不得按电平轮询；IO46 悬空。上表保留 2026-09-08 对账当时的记录，未改写历史结论。

> **更正（2026-09-17，追加）**：PWR 控制器由 **TPS3424 换为 LTC2954ITS8-1**（ADI 按键开关机控制器），随之三处语义/电气变更：①`PWR_INT` 由「短按 50 ms／长按 100 ms 脉冲」变为 LTC2954 `INT` 的**按键状态输出**——开漏低有效、10 kΩ 上拉至 `V3V3`，按下约 32 ms 后拉低并**在按住期间持续为低**，固件改以「边沿中断捕获 + 自行测低电平时长」判定短按/长按，不得按电平轮询；②长按开关机改为**纯硬件**（LTC2954 用 ONT/PDT 外接电容独立计时，**设计目标**约 1.76 s 开／2.18 s 关、时长待样机实测，到点自行切换 `EN`，不经 ESP32），固件不参与；③`PWR_STATE` 由 TPS3424 的**推挽输出**改为 LTC2954-1 `EN` 的**开漏输出**（100 kΩ 上拉至 `VSYS`，恰好一组、不得再并第二组）。网络名 `PWR_STATE`／`PWR_INT`／`TP_PWR_STATE`／`TP_PWR_INT` 与 IO8 分配均不变。上一条更正（2026-09-17）与上表保留各自作出当时的记录，未改写历史结论。
