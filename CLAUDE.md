...
- UX 交付前运行 `python3 lvgl-design/ui_text_hygiene.py`，失败时不得继续进入 SquareLine 或 uni-app 复刻。

## UX 真源

- PRD、产品简报、架构主干决定行为与数据语义。
- `DESIGN.md` 决定视觉身份，`EXPERIENCE.md` 决定行为体验。
- `UI_CONTRACT-device.md` 与 `UI_CONTRACT-miniapp.md` 决定 Pen/HTML 闭包。
- `.pen`、HTML 和前端实现不得自行发明产品状态或文案。
