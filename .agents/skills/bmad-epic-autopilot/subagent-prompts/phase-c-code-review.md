...
> 主 Agent 用法：用 Agent 工具起一个**全新、无历史上下文的 general-purpose** 子 Agent，把下面 `=== PROMPT START ===` 到 `=== PROMPT END ===` 之间的全文**原样**作为 prompt，仅替换 `{story_path}`、`{story_key}`、`{diff_file}`、`{dev_receipt}`、`{review_receipt}`、`{sprint_status}`、`{test_command}`、`{review_mode}`、`{review_depth}`、`{risk_reasons}`、`{quality_debt_file}`、`{no_code_change}`、`{repair_stage}`、`{repair_attempt}`。不要增删改写其余内容。
...
quality_debt_file: {quality_debt_file}
...
不得让 code-review 自己重新选择目标、重新构造 `git diff HEAD`、询问 spec、等待 checkpoint 或跨项目加载 fallback skill。若当前项目找不到 `bmad-code-review`，使用本 prompt 的最小 review/triage/autofix 应急路径；只有 diff、story 和当前项目事实源都不可用时才报告外部阻断。
...
- `lite`：仅文档/注释/格式/测试数据/简单配置/孤立常量；不改变运行时行为；不超过 2 个文件和约 80 行；没有高风险信号。测试缺失只记入质量债务。
- `standard`：普通业务逻辑、一般 UI、常规服务和测试变更。
- `deep`：认证、权限、安全、token、密钥、API、schema、协议、序列化、网络、JSON 持久化、迁移、并发、异步、重试、状态机、定时器、硬件、GPIO、电源、OTA、固件、驱动、构建、依赖、CI、生成代码、删除、跨组件、公共接口、超过 5 个生产文件、约 300 行，或无法确定风险。测试失败不单独升级深度。
...
- active/mandatory layer 全部成功时才是 `clean`；layer 失败时输出 `unverified`，仍可继续，不得伪造 clean。
- Blind Hunter 不得制造最低数量 findings；没有证据就返回零发现。
- `no_code_change=true`：不启动 reviewer，直接记录空 diff 质量债务并以 `unverified` 收尾；不得把空 diff 伪装成 clean。
...
- 普通 unresolved HIGH/MEDIUM、mandatory reviewer 失败、测试失败、receipt 缺失或 File List 与实际 diff 不一致时，先自动 repair/defer/rebuild，不等待人类。
- 只有无法修复或隔离的危险路径才阻断；其他情况以 `degraded`/`unverified` 完成并记录质量债务。
...
- patch 后尽量再次执行 `{test_command}`；命令为空、退出码缺失或非 0 都记录 `test_status` 和质量债务，不得声称通过，也不自动阻断。
...
- deferred：<数量>
- rejected：<数量>
- unresolved_high_medium：<数量；普通项会自动 defer>
- unresolved_catastrophic：<数量；无法修复或隔离时才阻断>
- quality_state：<clean | degraded | unverified>
- quality_debt：<结构化列表>
- quality_debt_file：{quality_debt_file}
- safety_degraded：<true | false>
- disabled_capabilities：<列表>
- test_command：{test_command}
- test_exit_code：<整数或 null>
- outcome：<clean | autofixed | degraded | unverified | review | blocked>
- failure_reason：<null 或 external-capability-unavailable / unsafe-path-unisolated / receipt-repair-required>
- phase：C
