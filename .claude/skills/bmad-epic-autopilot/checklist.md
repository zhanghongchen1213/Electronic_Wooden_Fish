...
主 Agent 在宣布某个 story `done`、循环取下一个之前，逐项核对。任一项不满足 → 先按实现、构建、证据、审查、降级恢复层修复；只有恢复层耗尽或外部能力不可用时才转 `steps/step-03-finalize.md` 失败分支。
...
- [ ] child 没有递归派发、没有内部 review→fix 循环；普通业务/证据失败由编排器按恢复层切换策略，直到完成或外部能力耗尽；明确的模型容量启动失败且零状态/零 receipt 不计入恢复次数，持续用全新 child 重试。
...
- [ ] 涉及 ESP-IDF 的 B receipt 记录每次构建日志、退出码、最早错误/环境诊断、修复和最终退出码；`127` 已先重新激活并重建，若仍失败则必须记录安全降级与禁用能力，不能把环境错误直接当作终止。
- [ ] 每次阶段都有机器 receipt；C 的 receipt 明确记录 `review_depth`、mandatory layers、failed layers、findings、质量状态、质量债务、测试退出码和 outcome。
- [ ] C 之前已持久化风险分类 JSON；receipt 经过 helper 结构校验，深度只能保持或向 `deep` 升级。
- [ ] `done` 只在实现或安全降级完成、构建门禁完成、scope/receipt/status 闭合且危险路径已修复或隔离时写入；测试失败/缺失和普通 reviewer 失败必须记录但不阻断。

## 失败与重试纪律
- [ ] 任一阶段未达期望状态时，按实现、构建、证据、审查和降级恢复层切换策略；每次都有新证据或新策略，不空转。
- [ ] `review` 未闭合时先进入 `repairing-review`/`repairing-evidence`，不等待人类；只有外部能力不可用或危险路径无法隔离才终止 Epic。
- [ ] 未知状态、坏 YAML、缺文件、空 diff、receipt 损坏均先尝试脚本恢复并保留备份。
