...
2. **先验证 sprint-status 输入。** `{sprint_status}` 正常时必须存在、可读、UTF-8 可解析，且 `development_status` 必须是 mapping。优先调用 `validate-status`；失败时先调用 `repair-status`，保存 `.repair-backup.yaml` 并从合法 receipt、story frontmatter 和文件名重建。只有恢复仍失败时才进入 step-03 的外部阻断分支。
...
5. **构建有序队列 `{story_queue}`。** 按**文件出现顺序**收集该区段内所有形如 `{epic_num}-<m>-<slug>: <status>` 的 story 行（排除 `epic-{epic_num}:` 自身和 `epic-{epic_num}-retrospective:`）。每项记 `{key, status}`。状态必须属于 `backlog`、`ready-for-dev`、`in-progress`、`review`、`done`、`blocked`；其他值先通过 receipt/story frontmatter 恢复，仍无法判断才进入外部阻断。
