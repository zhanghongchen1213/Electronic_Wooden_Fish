...
review_depth: '' # set at runtime: lite, standard, or deep
risk_reasons: [] # set at runtime from the caller's deterministic classifier
action_policy: '' # set at runtime: autofix or interactive
...
7. Sanity check: if `wc -l {diff_file}` exceeds approximately 3000 lines, warn the user and offer to chunk the review by file group. In unattended mode, return a structured `diff-too-large` failure instead of asking.
...
Present a summary before proceeding: diff stats (files changed, lines added/removed), `{review_mode}`, and loaded spec/context docs (if any). HALT and wait for user confirmation to proceed. Skip this checkpoint when `unattended=true` because the automation contract already fixed the inputs.
