...
TWO_MODE_REFERENCE_STATES = {
    ...
    "[UI][STATE][mode_layout][two_modes_small_step] 模式与电源·两模式参考（标准/小碎步）": {
        "ui_btn_mode_standard": (16, 96, 181, 128),
        "ui_btn_mode_sport": (213, 96, 181, 128),
        "ui_btn_exo_poweroff": (16, 282, 378, 112),
    },
    "[UI][STATE][mode_layout][two_modes_small_step_disconnected] 模式与电源·两模式断连参考（标准/小碎步）": {
        "ui_btn_mode_standard_selected_disabled": (16, 96, 181, 128),
        "ui_btn_mode_sport_disabled": (213, 96, 181, 128),
        "ui_btn_exo_poweroff_disabled": (16, 282, 378, 112),
    },
...
DOCUMENTATION_EXPORT_MARKERS = (
    ...
    "ui_rail_two_mode_small_step_reference",
    "ui_rail_two_mode_small_step_disconnected_reference",
    "ui_two_mode_small_step_nav_reference",
    "ui_two_mode_small_step_disconnected_nav_reference",
)
...
def validate(project_dir: Path, html_path: Path) -> dict:
    ...
    for state_name, expected_objects in TWO_MODE_REFERENCE_STATES.items():
        ...
        if state_node.attrs.get("data-export-to-squareline") != "false":
            add_error(
                errors,
                "two_mode_reference_export_boundary",
                "双模式参考稿必须显式标记为不导出 SquareLine",
                state=state_name,
            )
        ...
        disconnected = "disconnected" in state_name
        small_step = "small_step" in state_name
        secondary_label_name = (
            "ui_btn_mode_sport_label_disabled"
            if disconnected
            else "ui_btn_mode_sport_label"
        )
        secondary_icon_name = (
            "ui_icon_mode_sport_disabled"
            if disconnected
            else "ui_icon_mode_sport"
        )
        secondary_labels = descendants.get(secondary_label_name, [])
        secondary_icons = descendants.get(secondary_icon_name, [])
        expected_secondary_text = "小碎步" if small_step else "健身"
        expected_icon_name = "footprints" if small_step else "dumbbell"
        if (
            len(secondary_labels) != 1
            or normalized_text(secondary_labels[0])
            != expected_secondary_text
            or len(secondary_icons) != 1
            or secondary_icons[0].attrs.get("data-icon-name")
            != expected_icon_name
        ):
            add_error(
                errors,
                "two_mode_reference_secondary_scene",
                "双模式参考稿第二卡的文案与图标必须匹配场景配置",
                state=state_name,
                expected_text=expected_secondary_text,
                expected_icon=expected_icon_name,
            )
        if any(
            name in descendants
            for name in (
