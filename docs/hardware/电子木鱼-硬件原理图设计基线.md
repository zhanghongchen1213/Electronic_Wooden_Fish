...
| 使用状态 | 最终路径 | 设备行为 |
| --- | --- | --- |
| 仅 USB-C | `VBUS → BQ25895 → VSYS → TPS22965 → VMAIN → V3V3/4G/音频` | 可开机、显示、4G、播放；无电池也能运行 |
...
**音频电源锁定：** 音频电源固定为滤波后的 V3V3_A；NS4150B 的 VCC 使用从 `V3V3` 分出的 `V3V3_A`，在功放入口放磁珠或负载开关、10 µF + 100 nF 去耦，并让 ES8311 模拟地回流不经过 BQ 开关电流路径。这样充电时不会发生 OTG 升压启停，优先消除此前的低频噪声源。NS4150B 的音量、温升和失真仍必须按目标扬声器实测。

**系统电源开关：** TPS22965 的输入改接 `VSYS`，输出命名为 `VMAIN`；其 ON 由“VBUS 有效”与 TPS3424 锁存输出的硬件逻辑合取/或取决于所购器件的有效极性，必须保证 USB-only 时无需电池也能打开系统。PWR 长按仍由 TPS3424 完成硬关机，EN 只留测试点。
...
3. `V3V3_A` 经过滤波后给 NS4150B，`PA_EN` 仅在音频轨稳定后拉高。
...
| TPS22965 | 系统主负载开关 | IN=`VSYS`、OUT=`VMAIN` | `VMAIN` 供 4G、V3V3 和音频；输入/输出去耦靠近器件；不得让 4G 峰值经过细线 |
| TLV62569 | `VMAIN → V3V3` | IN=`VMAIN`、OUT=`V3V3` | 2.2 µH、输入 10 µF+100 nF、输出 2×22 µF；最低输入和掉压需按电池/USB-only 实测 |
| V3V3_A 滤波 | 功放低噪声供电 | `V3V3 → 磁珠/负载开关 → V3V3_A` | 功放入口 10 µF+100 nF；模拟地单独回流；不使用 BQ PMID 升压供音频 |
...
| U4 V3V3 稳压 | `VMAIN` | `V3V3` | 供 ESP32、显示逻辑、I²C、PVDF、RGB；电流和压降需测 |
| V3V3_A 滤波 | `V3V3` | `V3V3_A` | 只供 NS4150B；稳定后才允许 `PA_EN` |
...
- `ES8311`：共享 I²C；I²S `DOUT=IO13`、`WS=IO14`、`BCLK=IO18`、`MCLK=IO21`。codec 供电取 `V3V3`，模拟地/数字地按数据手册和 PCB 回流处理。
- `NS4150B`：`VCC=V3V3_A`，音频输入来自 ES8311 模拟输出，`PA_EN=IO48`。`PA_EN` 只有在 `V3V3_A` 稳定后才可拉高；静音、充电、故障时拉低。
...
ES8311 的 `I2C_SDA/SCL` 仍挂共享 `I2C0`，I²S 四根播放线只走音频路径；`V3V3_A` 只供功放，不把 5 V 直接送入 ES8311。
...
| P04 | ES8311、NS4150B、扬声器、RGB | `I2S_*`、`V3V3_A`、`PA_EN`、`RGB_DATA`、`SPK_P/N` |
...
- 4G `VMAIN`、USB VBUS、扬声器功放回路按峰值电流单独规划回流和铜宽；不得经 `V3V3` 或 `VSYS` 细线供电。
