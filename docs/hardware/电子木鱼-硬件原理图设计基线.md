...
| 项目 | 决策 | 依据/影响 |
| --- | --- | --- |
| ES8311 数据回传 | **方案 B：不连接 ES8311 ADC 输出，IO17 留空** | `main_control` 的 IO17 实际是 GPS UART TX；其音频 I2S 输入脚为 IO48，且是录音链路。本项目只有木鱼音播放，因此不再为录音保留 IO17。 |
| M100 NET_STATUS | 载板有该引脚，但 `main_control` 只配置 IO16 为输入高阻，未调用 `gpio_get_level` 读取；EWF 可不接 IO16 | 若保留兼容能力，IO16 输入高阻、禁用上下拉；若本项目不使用，ESP32 IO16 直接 NC。 |
| 按键 | 产品按键只有 **BOOT（GPIO0）** 和 **PWR（GPIO46）** | EN/RESET 不设产品按键，只留测试点；PWR 长按由 TPS3424 完成硬件开关机。 |
| USB MUX | 先保留 FSW7227 作为当前可行方案，另列“直连 ESP32 + BQ D+/D− 不参与检测”简化方案 | Type-C CC 只能识别供电角色/电流能力，不能替代 D+/D− 数据路径切换。 |
...
| 状态 | 允许行为 | 关键电源路径 |
| --- | --- | --- |
| 仅 USB-C | 整机运行，可播放，电池可充电 | `VBUS_5V → BQ25895`；系统经 `VBAT_SW/3V3` |
| 仅电池 | 整机运行，可播放 | `BAT_PROTECTED → TPS3424/TPS22965 → VBAT_SW → 3V3` |
| USB-C + 电池 | 整机运行、同时充电、可播放 | BQ25895 负责 power-path/充电；音频电源不能依赖不稳定的 OTG 切换瞬态 |

#### 方案比较

| 方案 | 做法 | 优点 | 风险/代价 | 判断 |
| --- | --- | --- | --- | --- |
| A：当前 BQ PMID + TPS22919 | 电池模式由 BQ OTG 产生 `PMID_OTG_5V`，再经 TPS22919 给 NS4150B | 可从单节电池得到 5V 功放电源 | BQ 充电/OTG 状态切换、开关纹波和地回流可能造成你之前遇到的低频噪声；需要严格时序和隔离 | 不建议作为首选 |
| B：NS4150B 直接使用 3V3 | 删除 `PMID_OTG_5V`、TPS22919，NS4150B VCC 接受控 `3V3_AUDIO` | 电路最简单；USB/电池/同时接入天然共用同一逻辑电源；少一个升压开关噪声源 | 3.3V 输出功率、扬声器音量和失真需按 NS4150B 3–5.25V 范围实测 | **首选验证方案** |
| C：USB VBUS 与电池升压 OR-ing | USB 时用 VBUS，电池时用独立 5V boost，通过电源 OR-ing 供功放 | 5V 功放余量最大 | 器件和噪声源增加；需要防反灌、切换瞬态和地回流设计 | 只有 B 方案音量不足时采用 |

**Type-C 与 FSW7227 的关系：** CC1/CC2 的 5.1 kΩ Rd 只用于告诉充电器“本设备是受电端”并识别插入/正反方向；它不能把同一组 D+/D− 自动分给 BQ 或 ESP32。FSW7227 的唯一用途是让 BQ25895 的 BC1.2/适配器识别和 ESP32 USB Serial/JTAG 共用一个 USB-C 口。

**更简单的 USB 方案：** 若决定不使用 BQ 的 BC1.2 自动识别，可将 USB-C D+/D− 直接接 ESP32 IO20/IO19，BQ25895 的 D+/D− 按其数据手册“不使用检测”方式处理（短接或留空，必须以所购版本手册为准），由 CC 和固定 `IINLIM/ILIM` 限制输入电流。这样可同时充电和 USB 通信，也删除 FSW7227 及选路焊盘；代价是失去 BC1.2 自动识别，需要固定配置充电输入电流并验证老式充电器兼容性。

**充电低频噪声的优先排查顺序：** 先确认 NS4150B 不从 BQ 开关节点或 PMID 切换瞬态取电；再将 ES8311 模拟地、功放地、BQ 开关电流回路和 4G 回流分区；最后检查 `PA_EN`、音频输入耦合电容和充电电流变化时的地电位。没有示波器和扬声器端频谱证据前，不能承诺“边充边播无噪声”。
...
ES8311 的 `I2C_SDA/SCL` 仍挂共享 `I2C0`，I²S 四根播放线只走音频路径；`AUDIO_5V` 只供功放，不把 5 V 直接送入 ES8311。
