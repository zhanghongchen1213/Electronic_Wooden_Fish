...
| --- | --- | --- | --- |
| 启动绑带输入 | IO0、IO46 | 允许作输入；复位采样窗口必须保持下载要求电平 | 已确认；外设不得反驱，需上电波形验证 |
...
| 音频/4G/状态灯 | IO3、IO10、IO13–18、IO21、IO43–44、IO48 | 允许作数字输出、I²S 或 UART；IO43/44 不得再作 console | 已确认；电平、时序、峰值负载需实测 |
| USB-Serial-JTAG | IO19、IO20 | 允许作原生 USB D−/D+；不启用同 PHY USB-OTG | 已确认；PC 枚举与烧录需实测 |
| 保留脚 | IO22–37、IO45 | 禁止作为项目外设 GPIO；IO45 不接 IMU INT2 | 已确认；PCB 不得布线占用 |
...
1. **原理图冻结前**：按 GPIO 合同和网络清单完成逐脚 ERC 预审，确认 FPC、M100 载板和器件封装资料。
2. **首版 PCB**：完成电源分轨、USB/QSPI/I²C/音频/PVDF/4G 布局评审，导出网表和 BOM；记录未关闭开放项。
...
5. **签收门禁**：归档示波器/电子负载/实物插合证据，更新 JSON 与开放项状态；在此之前不得宣称 ERC、DRC 或样机通过。
