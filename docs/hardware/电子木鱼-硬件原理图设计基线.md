...
| 显示 BTB 资料 | `docs/hardware/参考原理图/3593896291PY206-W38-V2(2).pdf` | PY206 模组对外 24-pin 针脚；排线端公座 OK-14GM024-04，主板端母座 OK-14F024-04；CO5300/CST9217/OCP21351 均在模组内部 |
...
**系统电源开关：** TPS22965 的输入改接 `VSYS`，输出命名为 `VMAIN`；TPS3424 VDD 接 `VSYS`，其 pin 6 `RESET`（高有效锁存）形成 `SYS_EN` 并驱动 TPS22965 `ON`，同时与 ESP32 IO46 的 `PWR_STATE` 输入同网。这样 USB-only 无电池时也能按键上电；PWR 长按由 TPS3424 硬件释放锁存，ESP32 的 `EN/RESET_N` 仍只留独立复位测试点。
...
| TPS3424 | PWR 长按检测与硬件锁存 | `VSYS`、PWR 按键 → `PB`；`RESET` → `SYS_EN` | pin 8 `KILL` 接 `VDD`；长按关断不依赖 ESP32；SPT/LPT 按料号和电容计算 |
...
| V3V3_A 滤波 | 功放低噪声供电 | `V3V3 → FB1(BLM21PG121SN1D/C79382) → V3V3_A` | FB1 为 0805、120 Ω@100 MHz、3 A、30 mΩ 磁珠；只供 NS4150B；功放入口 22 µF×2；ES8311 不得挂此网；不使用 BQ PMID 或 `VMAIN` 直供音频 |
...
以下针脚来自仓库内 `docs/hardware/参考原理图/3593896291PY206-W38-V2(2).pdf` 第 3–4 页（型号/修订与实物接触方向仍需确认）。`TP-*` 为 CST9217 触摸侧，`L-*` 为 CO5300 QSPI 侧。PDF 的 `TP PIN定义` 是模组上的测试焊盘，不是第二个必须插接的 6-pin 外部连接器。
...
| 8 | `VBAT` | `VLCD`（首版接 `VMAIN`） | AMOLED 模拟/面板供电 | **推荐初值**；范围/纹波[必须样机验证] |
| 9 | `MTP/NC` | `NC` | 不接 | **已确认** |
...
| U4 V3V3 稳压 | `VMAIN` | `V3V3` | 供 ESP32、显示逻辑、I²C、PVDF、RGB、ES8311 `AVDD`/`DVDD`/`PVDD`；电流和压降需测 |
...
FPC pin 1、接触面和 pin 8 `VLCD` 在拿到实物前只算推荐目标；任何“针脚连续”证据都不能替代供电时序和首帧波形。
...
| U3 | TPS22965DSGR | C122837 | 1 | 推荐初值 | UDFN-8-EP 2×2；VMAIN |
| U4 | TLV62569DBVR | C141836 | 1 | 推荐初值 | SOT-23-5；V3V3 降压 |
...
| U13 | CW2015CHBD | C881838 | 1 | 推荐初值 | TDFN8-EP 2×3 mm；地址 0x62 候选；见 [CW2015CHBD 设计文档](外围电路设计/CW2015CHBD外围电路设计与接线.md) |
| U14 | QMI8658A | 待核 | 1 | 推荐初值 | SA0=高，0x6B；MVP 默认不启用 |
...
| J2 | B3B-PH-K-S(LF)(SN) | C131339 | 1 | 已确认接口 | 电池 3P PH 2.0 mm，候选 805080/4000 mAh 或 904260/3000 mAh，内置 NTC R25=10 kΩ；β 值待确认 |
...
- FPC 目标资料：`docs/hardware/参考原理图/3593896291PY206-W38-V2(2).pdf`（第 3–4 页针脚/示意；目标版本与方向仍待实物核对）。
