...
| 按键 | 产品按键只有 **BOOT（GPIO0）** 和 **PWR（IO8）** | EN/RESET 不设产品按键，只留测试点；PWR 长按由 TPS3424 完成硬件开关机；PWR 输入走 `PWR_INT` 脉冲，固件以边沿中断读取。 |
...
| PWR_INT | IO8 | input_interrupt | 否 | 允许；复用由 GPIO matrix/外设约束决定 | 接 TPS3424 pin 7 `INT`，开漏输出由 10 kΩ 上拉至 `V3V3`；固件必须用 GPIO 边沿中断捕获 50 ms/100 ms 脉冲，不得按电平轮询；PWR 长按由 TPS3424 完成硬开关机 |
...
**系统电源开关：** TPS22965 的输入改接 `VSYS`，输出命名为 `VMAIN`；TPS3424 VDD 接 `VSYS`，其 pin 6 `RESET`（高有效锁存）形成 `PWR_STATE` 并驱动 TPS22965 `ON` 与 TLV62569 的 `EN`；`PWR_STATE` 是推挽输出，其高电平为 `VSYS`（最高 4.2 V），因此不接任何 ESP32 引脚。按键输入另走 TPS3424 pin 7 `INT` 的 `PWR_INT`（开漏、10 kΩ 上拉至 `V3V3`）到 ESP32-S3 的 IO8，由固件以边沿中断读取。这样 USB-only 无电池时也能按键上电；PWR 长按由 TPS3424 硬件释放锁存，ESP32 的 `EN/RESET_N` 仍只留独立复位测试点。
...
| TPS3424 | PWR 长按检测与硬件锁存 | `VSYS`、PWR 按键 → `PB`；`RESET` → `PWR_STATE`（TPS22965 `ON` 与 TLV62569 `EN`）；`INT` → `PWR_INT`（ESP32 IO8） | pin 8 `KILL` 接 `VDD`；长按关断不依赖 ESP32；SPT/LPT 按料号和电容计算 |
...
**职责：** 由 BQ25895 建立 USB/电池 power-path，由 TPS3424/TPS22965 控制 VMAIN；音频从 V3V3 分出独立滤波轨。

| 模块 | 输入 | 输出/连接 | 设计判断 |
| --- | --- | --- | --- |
| BQ25895 | `VBUS`、`VBAT` | `VSYS`、充电状态 I²C | 负责 NVDC/充电和系统 power-path；不使用 OTG |
| TPS3424 + TPS22965 | VSYS、PWR 按键/VBUS 有效 | `VMAIN` | 长按断电由硬件完成；USB-only 仍可上电 |
...
| PWR 长按锁存 | TPS3424A11C13ADRLR | 待核 | 1 | 推荐初值 | DRL/SOT-5X3；pin 6 `RESET` → `PWR_STATE`，pin 7 `INT` → `PWR_INT`；按版本核对时序 |
...
| 页码 | 页面 | 必须出现的网络/测试点 |
| --- | --- | --- |
| P01 | USB-C、BQ25895、TPS3424/TPS22965、电池与电源分轨 | `VBUS`、`VBAT`、`VSYS`、`VMAIN`、`V3V3`、`V3V3_A`、`TP_VSYS`、`TP_VMAIN` |
