...
建议按“总览 → GPIO/总线 → 电源 → 模块 → 走线 → 验收”的顺序阅读。每个结论都带有状态：**已确认**是项目接口合同，**推荐初值**是首版落图候选，**必须样机验证**是尚未用实物、示波器、电子负载或 EDA 报告关闭的项目。

### 0.1 一张图看懂整板

```mermaid
flowchart TB
  USB["USB-C J1\n5V / D+ / D- / CC"] --> PWR["充电与电源\nBQ25895 + TPS3424 + TPS22965"]
  BAT["单节受保护电池 J2"] --> PWR
  PWR --> HV["VBAT_SW\n4G 高电流轨"]
  PWR --> LOGIC["3V3\n主控与逻辑"]
  PWR --> OTG["PMID_OTG_5V\n受控音频轨"]
  HV --> MODEM["Air780EGP / M100\nUART + SIM + 天线"]
  LOGIC --> MCU["ESP32-S3-WROOM-1-N16R8\nUSB-Serial-JTAG"]
  LOGIC --> DISPLAY["CO5300 AMOLED + CST9217\n24-pin FPC"]
  LOGIC --> SENSOR["CW2015 + QMI8658C\n共享 I2C"]
  LOGIC --> PVDF["PVDF\nADC IO9 + 唤醒 IO11"]
  OTG --> AUDIO["ES8311 + NS4150B\n扬声器"]
  USB --> MUX["FSW7227\nBQ/ESP32 数据单选"]
  MUX --> MCU
```
...
| 问题 | 当前结论 | 还差什么证据 |
| --- | --- | --- |
| ESP32 GPIO 能不能用 | 已用 32 个普通 GPIO 无重复；IO35–37 属 N16R8 的 Octal PSRAM，IO45 不使用 | 模组批次、启动波形和实际电平仍需核对 |
| 4G 会不会拖垮系统 | Air780EGP 只从 `VBAT_SW` 取电，不经过 `3V3` 或 `BQ_SYS` | 弱网发射峰值、压降和重启裕量 |
| USB 会不会互相反灌 | FSW7227 在 BQ 和 ESP32 两条数据路径中单选，禁止并联 | 三种选路状态的插拔与反灌实测 |
| 屏幕接口是否完整 | 24-pin FPC、QSPI、触摸 I²C、`LCD_VBAT`、`LCD_EN` 均已列出 | FPC 正反面、pin 1、供电范围和首帧波形 |
| 文档是否代表板子已通过 | 不是；当前是可落图和可审查基线 | ERC、PCB DRC、打样和样机证据 |

### 0.3 模块阅读入口

- **屏幕/触摸**：先看 §5 FPC 映射，再看 §6.2 显示与触摸布局要求。
- **音频**：看 §6.3，重点是 `AUDIO_5V → NS4150B → SPK` 和 `PA_EN` 时序。
- **电量计/IMU/PVDF**：看 §6.4，重点是共享 I²C、IO41 中断和 IO9/IO11 模拟/唤醒分工。
- **电源及外围**：看 §2 的电源图、§2.1 网络表和 §6.1 电源器件候选。
- **USB-C 及外围**：看 §2.2 MUX 说明和 §6.6 USB/连接器走线要求。

### 0.4 章节导航

| 章节 | 解决的问题 |
| --- | --- |
| §1 目标、官方依据与边界 | 哪些资料可以作为依据，哪些旧设计明确不采用 |
| §2 总览、电源网络与 USB MUX | 电从哪里来、经过什么器件、最后供给谁 |
| §3 GPIO 合同 | 每个 IO 是否可用、是否受启动或模组内部资源限制 |
| §4 总线实例与地址 | 哪些设备共享总线，是否存在资源或地址冲突 |
| §5 屏幕 FPC | 24 根 FPC 针脚分别连接什么 |
| §6 模块深入说明 | 按电源、屏幕、音频、传感器、4G、USB 分模块评审 |
| §7 EDA 落图 | 原理图分层页和统一网络名 |
| §8 验收、走线与开发路线 | 何时算 ERC、DRC、样机和量产准备完成 |

## 1. 目标与边界

### 1.0 官方资料与审查口径

本基线以 Espressif 官方 **ESP32-S3-WROOM-1** 数据手册、ESP32-S3 技术参考手册（GPIO、strapping、USB-Serial-JTAG、SPI0/1 Flash/PSRAM 保留脚）为器件约束依据：
...
- [ESP32-S3 硬件设计指南：原理图检查清单（Espressif）](https://docs.espressif.com/projects/esp-hardware-design-guidelines/en/latest/esp32s3/schematic-checklist.html)
- [ESP32-S3 硬件设计指南：PCB 布局（Espressif）](https://docs.espressif.com/projects/esp-hardware-design-guidelines/en/latest/esp32s3/pcb-layout-design.html)

官方资料只用于判断引脚能力与启动风险；器件地址、FPC 针脚、电源电压和外围值仍以项目网络清单、spine、目标器件资料及样机证据为准。下文的“已确认”表示项目合同冻结，不表示 ERC、DRC 或样机通过。
...
网络清单中的 32 个普通 GPIO 均落在项目允许集合（IO0–21、IO38–44、IO46–48）内，未使用 GPIO22–37 与 GPIO45 等保留脚；`EN` 为复位专用输入，不计入普通 GPIO。逐项方向、约束和验证状态见下表。结论按官方能力归类：

| 类别 | 项目 GPIO | 合法性结论 | 启动/验证门禁 |
| --- | --- | --- | --- |
| 启动绑带输入 | IO0、IO46 | 允许作输入；正常 SPI 启动为 IO0=1，联合下载模式为 IO0=0 且 IO46=0 | 已确认；外设不得反驱，需上电波形验证 |
| ADC/唤醒输入 | IO9、IO11 | 允许作 ADC1_CH8 与低功耗唤醒输入 | 已确认；PVDF 模拟范围、极性和误触发必须实测 |
| 共享 I²C | IO1、IO2 | 允许作开漏双向总线，单一仲裁与共享上拉 | 已确认；速率与上拉值需实测 |
| QSPI/显示 | IO4–7、IO12、IO40、IO42、IO47 | 允许作显示控制与数据输出 | 已确认；FPC 方向、首帧时序需实测 |
| 启动绑带/状态灯 | IO3 | 允许作 RGB 输出；默认 eFuse 未配置时 JTAG 选择忽略 IO3，若启用 `EFUSE_STRAP_JTAG_SEL` 必须提供确定电平 | 已确认；eFuse 状态和复位电平需核对 |
| OTG/音频/触摸/IMU/4G | IO8、IO10、IO13–18、IO21、IO38–39、IO41、IO43–44、IO48 | 允许作数字输出、输入、I²S、UART 或 GPIO matrix 信号；IO43/44 不得再作 console | 已确认；电平、时序、峰值负载需实测 |
| USB-Serial-JTAG | IO19、IO20 | 允许作原生 USB D−/D+；不启用同 PHY USB-OTG | 已确认；PC 枚举与烧录需实测 |
| 保留脚 | IO22–37、IO45 | 禁止作为项目外设 GPIO；其中 IO35–37 在 N16R8 模组连接 Octal PSRAM，IO45 为 VDD_SPI 启动绑带，不接 IMU INT2 | 已确认；PCB 不得布线占用 |

若后续设计需要变更任一 GPIO，必须先更新 spine 与 JSON，再同步本表和 BSP；不得在固件中静默重映射。

**逐脚补充结论：** `IO8=BQ_OTG_EN` 为普通输出，不能连接 M100 `GNSS_VCC`；`IO38/39` 为 CST9217 复位/中断，`IO41` 为 QMI8658C INT1，三者均不是启动绑带；`IO3` 虽作为 RGB_DATA，仍必须保留 GPIO3 的 JTAG strapping 说明。Espressif 模组表明确指出 N16R8 的 IO35–37 已连接 Octal PSRAM，不可作其他用途；GPIO0/3/45/46 在复位时采样，采样锁存后才恢复普通 GPIO。
...
| 实例 | 正式用途 | 信号/引脚 | 资源判断 | 走线要求 |
| --- | --- | --- | --- | --- |
| I2C0 | 共享配置与状态 | SDA=IO1、SCL=IO2 | 1 条总线、BSP 单一仲裁；地址必须无冲突 | 单组上拉、分支短、预留 SDA/SCL 测试点 |
| I2S0 | ES8311 音频 | MCLK=IO21、BCLK=IO18、WS=IO14、DOUT=IO13、DIN=IO17 | 5 根 GPIO 可用，不与 QSPI/USB 重叠 | 时钟与数据成组，远离功放开关节点 |
| SPI3/QSPI | CO5300 AMOLED | CS=IO40、SCLK=IO5、D0=IO6、D1=IO7、D2=IO12、D3=IO42 | 1 条高带宽显示链路；不占内部 SPI0/1 | 等长成组、连续参考地，预留串阻料位 |
| UART | Air780EGP AT | TX=IO43、RX=IO44；DTR=IO10、RST=IO15、状态=IO16 | 4G 独占 AT 入口；IO43/44 不作 console | TX/RX 交叉、确认载板电平，靠近连接器预留 TP |
| USB-Serial-JTAG | 烧录/CDC/OpenOCD | D−=IO19、D+=IO20 | 原生 USB 唯一调试通道；不启用 USB-OTG | 差分等长、连续地参考、ESD 靠近连接器 |

ESP32-S3 外设可经 GPIO matrix 路由，但本项目不因“可路由”而随意换脚：GPIO 合同优先保证启动绑带、Octal PSRAM、USB 和 4G 单一所有权。
...
布局时将 BQ 开关电感、输入/输出电容、TPS22965 和 4G 电源入口放在同一高电流区域；3V3、PVDF 和音频模拟地从开关节点旁边绕开。每条轨都要有测试点，详见 §2.1 与 §8.1。
