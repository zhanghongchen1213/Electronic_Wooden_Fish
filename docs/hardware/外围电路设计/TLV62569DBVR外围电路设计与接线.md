...
- 输出电容为 `V3V3` 网上的 **2 颗 22µF**，封装为 **0603 与 0402 各 1 颗**；`V3V3_A` 去耦为 **2 颗 0603**。（原理图 `C0805/22uF` 共 3 只且全部落在 `VSYS` 网，不在本器件输出侧；见 §3.4 与第 4 节）
...
| `C45783` 与 `C30926` 的全板需求口径 | 跨文档修改 | 备料数量 | 原理图 `C0805/22uF` 共 3 只全在 `VSYS` 网、`C0603/100nF` 共 11 只；BQ25895 与 TPS22965 两份文档仍记 `C45783` 全板 5 颗、`C30926` 与本表口径不一致，须按原理图改正，见第 4 节 |
...
flowchart LR
  ...
  L --> V3V3[V3V3]
  V3V3 --> COUT1[输出电容 22µF 0603]
  V3V3 --> COUT2[输出电容 22µF 0402]
  COUT1 --> GND
  COUT2 --> GND
...
选用 **2 × 22 µF = 44 µF**（`V3V3` 网上 2 颗，封装 0603 与 0402 各 1 颗），在 10–47 µF 内，且 Table 4 认可。手册第 9 页 Table 4 脚注 (2) 说明有效容量可在 +20%／−50% 内变化，即有效容量最低约 22 µF，仍落在手册允许区间内。
...
| `C2` | 输出电容 | `V3V3` 网上 2 × 22µF（0603 与 0402 各 1 颗，见 §3.4） | 已具备 |
...
| 输出电容 | 2 | `V3V3` 网上 2 颗 22µF（0603 件 CL10A226MO7JZNC，22µF/0603；0402 件料号待定），各 1 颗 | 0603 件 C2762594；0402 件未检出 | 0603 件可用 3（在库 8 − 占用 5）；0402 件未检出 | 库存未检出，待查 |
...
**输出电容封装与 `C45783` 的口径（按设计修正）**：本板中 `C0805/22uF` 共 3 只且 3 只全部落在 `VSYS` 网；`C0603/22uF` 共 3 只（`V3V3` 网 1 只、`V3V3_A` 网 2 只）；`C0402/22uF` 共 3 只（`V3V3` 网 1 只、`V3V3_A` 网 2 只）。因此本器件 `V3V3` 输出侧的 2 颗 22µF 是 **0603 与 0402 各 1 颗，不是 0805**；`C45783`（22µF/0805）的 3 颗全部属 `VSYS` 网去耦用料，本器件不认领，其全板需求与缺口见下方合计。
...
- 输出电容：原理图 `V3V3` 网上的 2 颗 22µF 封装为 0603 与 0402；0603 件按 C2762594 领用，0402 件料号待定，需先选定并采购。`C45783`（0805）不在本器件输出侧，本行不认领。
...
TLV62569 单板用料：稳压器 1 件 + 电感 1 件 + 电容 6 颗 + 电阻 2 颗 + 磁珠 1 件。

跨文档共用料号（按嘉立创编号 + 用途合并，跨本目录全部文档）：

- `C3040393`（2.2µH 电感）：**全板 2 颗**（BQ25895 与 TLV62569 各 1），可用 6 颗，满足。
- `C15850`（10µF/0805）：全板 5 颗，可用 21 颗，满足。
- `C30926`（100nF/0603）：原理图 `C0603/100nF` 共 11 只，**全板 11 颗**，可用 89 颗，无缺口。
- `C25803`（100kΩ/0603）：全板 3 颗（LTC2954 两处 + 本器件 FB 下分压），可用 83 颗，满足。
- `C45783`（22µF/25V/0805）：原理图 `C0805/22uF` 共 3 只且全部落在 `VSYS` 网，**全板需求 3 颗，可用 2 颗，缺口 1 颗**。该料号在 BQ25895 与 TPS22965 两份文档中各记 3 颗，实为 `VSYS` 上同一组电容，不得重复累计；本器件 `V3V3` 输出侧的 2 颗 22µF 封装为 0603 与 0402，不属该料号，不计入。
- `C2762594`（22µF/16V/0603）：全板 3 颗（`V3V3_A` 去耦 2 颗 + `V3V3` 输出 0603 件 1 颗），可用 3 颗，满足。
...
| 未检出 | 22µF/0402 | `V3V3` 输出电容（0603 件按 C2762594 领用） |
