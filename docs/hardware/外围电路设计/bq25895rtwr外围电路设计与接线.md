...
> 电源网络统一命名见 [`../电源网络命名规范.md`](../电源网络命名规范.md)；`VBUS`、`VBAT`、`VSYS`、`VMAIN`、`V3V3`、`V3V3_A` 是本文使用的板级正式网络名。
...
| BQ 输出尚未接入后级 | `VSYS` 仅有 `C17.2 C18.2 U2.15 U2.16 U14.2`；U8、U9 出现在封装列表，但没有任何网络端点 | 后级设计阶段补齐 `VSYS → TPS22965 → VMAIN → TLV62569 → V3V3`，以及 `V3V3 → V3V3_A`（仅 NS4150B）和 ES8311 接 `V3V3`。此项属于未完成接口，不是要求把 SYS 直接短接 BAT、V3V3、VMAIN 功放或 M100。 |
...
不要将 CN2.3 直接接 GND，也不要接 VBAT。不应在 CN2.3 与 U2.11 之间再串一个 10kΩ 电阻。R11 是温度窗口调整电阻，不是用于替代 NTC，保留它。

## 4. 全部引脚和分区接线图

以下分区图一起覆盖 U2 的全部外部连接。图中实线仅表示电气连接，不表示电流方向；电容均是并联支路。参考号 U2 是本次 BQ，U1 是 ESP32，不要求用户改位号。

### 4.1 VBUS、VBAT、VSYS 与 BQ 局部节点

```mermaid
flowchart LR
  V["VBUS<br/>F2.1"] --- P1["U2.1 VBUS"]
  V --- C13["C13 1uF<br/>库存目标 50V X7R 0805"] --- G["GND"]
  V --- C14["C14 100nF<br/>库存目标 50V X7R 0603"] --- G
  V --- C15["C15 10uF<br/>库存目标 25V X5R 0805"] --- G
  PMID["VPMID（U2.23 PMID）<br/>网表 $1N264"] --- C6["C6 10uF<br/>25V X5R 0805；有效容量待核"] --- G
  SW["SW_BQ 开关节点<br/>网表 $1N253"] --- P19["U2.19 SW"]
  SW --- P20["U2.20 SW"]
  ...
  B["CN2.1 VBAT"] --- BAT["U2.13 / U2.14 BAT"]
  B --- C16["C16 10uF<br/>库存目标 25V X5R 0805"] --- G
  G --- EP["U2.17 / U2.18 PGND<br/>U2.25 EP：最新网表已接地"]
  G --- J2["CN2.2 电池负端"]
```

VBUS、VBAT、VSYS、VPMID 和 SW_BQ 五个节点不可外部互相短接。器件内部 MOSFET 完成相应电源路径；VPMID 必需去耦，即使不用 OTG 也不能悬空不装电容。

### 4.2 REGN 与 BTST

```mermaid
flowchart LR
  R["VREGN（U2.22 REGN）<br/>网表 $1N256"] --- C19["C19 4.7uF<br/>库存目标 25V X5R 0603"] --- G["GND，模拟回流"]
  R --- TS["U15.1<br/>进入 §3 的 TS 分压"]
  B["BTST_BQ（U2.21 BTST）<br/>网表 $1N252"] --- C20["C20 47nF<br/>库存目标 16V X7R 0603"]
  C20 --- S["SW_BQ（U2.19 / U2.20 SW）<br/>网表 $1N253"]
```

C20 不接地。REGN 不作为 V3V3 输出使用。原理图统一 GND；布局时把模拟回流与开关回流分开引到芯片附近的公共地/PowerPAD，不应只增加一个未连接的 PGND 标签。

### 4.3 I²C、控制和未使用引脚

```mermaid
flowchart LR
  V["V3V3"] --- R2["R2<br/>实物 C25804：10kΩ/0603<br/>网表仍为旧1kΩ/0402"]
...
- C17+C18（VSYS）：44uF nominal；只计 −20% 容差为35.2uF。完整工作条件下总有效值须≥20uF。
...
按名义参数，VSYS 电容的开关纹波估算 `ΔV ≈ ΔIL/(8fCeff)`，取0.379A、20uF 得约1.58mV；不包含 ESR、布局和负载阶跃。电容纹波 RMS 约 `ΔIL/(2√3)`，上述例约0.109A。PMID 电容 RMS 约 `IL_AVG√(D(1−D))`，D=0.5 时达平均电感电流的一半。容值合格不等于纹波/温升和瞬态通过。
...
5. 后级设计完成后检查VSYS到VMAIN及V3V3路径；再验证USB-only、电池-only和同时接入的电压、电流、温升与插拔瞬态。

布局按BQ手册第55页：PMID电容贴近pin23和PGND；SW双脚到电感输入最短，避免大面积开关铜；VSYS电容贴近电感输出和芯片；REGN/TS模拟回流避开功率回流；PowerPAD可靠焊接并用导热过孔连接地平面。
