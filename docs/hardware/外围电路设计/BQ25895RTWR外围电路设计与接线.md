...
- `C45783`（22µF±20%/25V/X5R/0805）：本芯片需 **3 颗**（`SYS` 去耦），即全板全部 3 颗（本板 `C0805/22uF` 共 3 只，全在 `VSYS` 网），库存可用 **2 颗**，**全板缺 1 颗**。`V3V3` 输出侧的 2 颗 22µF 封装为 0603 与 0402，属其它料号，不计入。
- `C15850`（10µF±10%/25V/X5R/0805）：全板 6 颗（`PMID`、`VBUS`、`VBAT`、`VLCD` 各 1，`VMAIN` 2），可用 21 颗，满足。
