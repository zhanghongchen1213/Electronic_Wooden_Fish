...
> 核验依据网表：`Netlist_Schematic1_2026-09-17 (2).tel`，导出 2026-09-17 14:35:05，SHA-256 `6fce0399be028c34ebdbb5e66b40d30e8e077642faf43a764c280f5d2320799e`
...
- **网表**：`Netlist_Schematic1_2026-09-17 (2).tel`（导出 2026-09-17 14:35:05，SHA-256 见文首）。`$PACKAGES` 提供元件值与封装名，`$NETS` 提供端点连接。同日出过三份导出，本版只依据 mtime 最新的一份；11:12 与 13:08 两份已弃用。网表按次核对、不归档，文中哈希对应上述那一次导出。
- **库存**：`库存列表-20260913211039.xlsx`，快照 2026-09-13 21:10:39。
- **嘉立创页面**：2026-09-17 实时核对（见第 4 节）。

### 1.2 核验范围

BQ25895（WQFN-24 + PowerPAD，25 个端点）的全部引脚，以及网表内与之相连的全部外围元件：8 类陶瓷电容、7 个电阻、1 个功率电感、VBUS 输入保护（自恢复保险丝与 ESD 管）、电池连接器引脚。
...
| 全部元件的耐压等级在网表中不可见 | 跨文档修改 | 本版按嘉立创页面实时核对的型号判定（见第 4 节）。更换料号时必须重新核对耐压与介质下限 | 嘉立创商品页，或 BOM 导出 |
...
| 脚号 | 名称 | 网络 | 接法 | 状态 |
|---:|---|---|---|---|
| 1 | VBUS | `VBUS` | 经自恢复保险丝接 USB-C VBUS；近端 1µF 去耦到 `GND`，另并 100nF 与 10µF 到 `GND` | 符合 |
| 2 | D+ | — | 不连接。本设计不接 USB 数据线，不使用 BC1.2 与 MaxCharge 检测 | 符合 |
| 3 | D− | — | 不连接，同 pin 2 | 符合 |
| 4 | STAT | 开漏充电状态 | 经 10kΩ 上拉到 `V3V3`；未接主控，也未接 LED | 符合 |
| 5 | SCL | `I2C_SCL` | 经 10kΩ 上拉到 `V3V3`，接 ESP32-S3 IO2；与 CW2015、CST9217、ES8311、QMI8658A 共享总线 | 符合 |
| 6 | SDA | `I2C_SDA` | 经 10kΩ 上拉到 `V3V3`，接 ESP32-S3 IO1；共享总线同上 | 符合 |
| 7 | INT | 开漏中断 | 经 10kΩ 上拉到 `V3V3`；未接主控 | 符合 |
| 8 | OTG | `GND` | 接地。OTG 引脚为低电平，即使 `REG03.OTG_CONFIG` 复位值为 1 也不会进入升压模式（手册第 4 页与第 19 页均为「OTG pin HIGH **and** OTG_CONFIG = 1」的与条件） | 符合 |
| 9 | CE | `GND` | 接地。手册第 4 页要求「CE pin must be pulled High or Low」，低电平即允许充电（需 `CHG_CONFIG = 1`） | 符合 |
| 10 | ILIM | 输入限流设定 | 经 200Ω±1% 到 `GND`；`IINMAX = KILIM / RILIM`，见 §3.1 | 符合 |
...
| 22 | REGN | `REGN` | 4.7µF 到 `GND`；同时作为 TS 分压上臂的偏置源 | 符合 |
| 23 | PMID | `PMID` | 10µF 到 `GND` | 符合 |
| 24 | DSEL | 开漏检测指示 | 经 10kΩ 上拉到 `V3V3`。因 D+/D− 未使用，该引脚的复用切换功能实际无效 | 符合 |
| EP(25) | PowerPAD | `GND` | 与 pin 17/18 同网 | 符合 |
...
- 功率电感：EDA 属性中补写感值与型号，使网表可自证；按 §3.2 实测温升。
