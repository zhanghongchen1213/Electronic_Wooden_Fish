...
| BOOT 按键通断与四脚内部连接 | 样机实测 | 按下能否可靠把 `IO0` 拉低、松开是否开路未确认 | 断电用万用表测按键两侧：松开开路、按下近 0 Ω |
...
| 原理图网络名 `RESET` 与正式网络名 `RESET_N` 不一致 | 跨文档修改 | 测试点 `TP_RESET_N` 按《电源网络命名规范》§4「`TP_` + 正式网络名」反推正式网络名应为 `RESET_N`，当版原理图的网络名仍为 `RESET`；落图、固件与 PCB 会出现两个名字 | 在 EDA 把该网络改名为 `RESET_N` 后重新导出原理图；本文按正式网络名 `RESET_N` 表述 |
...
| 脚号 | 名称 | 接法 | 状态 |
|---:|---|---|---|
| 3 | EN | `RESET_N`（原理图网络名 `RESET`）：10 kΩ 上拉至 `V3V3`；1 µF 至 GND；测试点 `TP_RESET_N` | 符合 |
| 16 | IO46 | 悬空，不接任何网络（启动绑带默认值由模组内部弱下拉给出 0） | NC |
| 27 | IO0 | `BOOT0`：10 kΩ 上拉至 `V3V3`；BOOT 按键跨接至 GND（按键 pin 1 接 `BOOT0`，pin 3 接 GND）；测试点 `TP_BOOT0` | 符合 |
...
- **BOOT 按键（C722964）**：编号取自库存快照且为可用状态，无采购待办；按键通断与四脚内部连接属样机实测项。
...
- **C26413（1µF/0603）**：本对象认领 1 颗（EN 复位电容）。同料号其他认领为 [`CW2015CHBD外围电路设计与接线.md`](CW2015CHBD外围电路设计与接线.md) §4 的 1 颗与 [`WS2811外围电路设计与接线.md`](WS2811外围电路设计与接线.md) §4 的 1 颗；本板中 0603 1µF 共 4 颗，第 4 颗落在共享 `V3V3` 网络、无法归属单一器件，不计入任何文档认领。全板合计 3 颗认领 + 1 颗共享，可用 241 颗，**无缺口**。
- **C722964（BOOT 按键）**：与 PWR 按键同料号（TSW063G43-250K / C722964），**全板需 2 件**，本表只计 BOOT 侧 1 件；可用 16 件，**无缺口**。
